/*
 ...
 Link to Problem : https://www.spoj.com/problems/ANDROUND/
...
void build(int node,int s,int e){
    ...
    //cout << (T[node<<1] & T[node<<1|1]) << endl;
    T[node]=T[node<<1] & T[node<<1|1];
}
int query(int node,int s,int e,int l,int r){
    if(e<l || s>r) return SET; //no overlap 
	...
	//freopen("oc1.txt", "w", stdout);
 	IOS;
       TC{
        ...
        arr[0]=left;
        arr[n+1]=right;
        rep(i,1,n+1)arr[i]=a[i-1];

        //rep(i,0,n+2)cout << arr[i] << ' ';cout<<endl;
        build(1,0,n+1);
        //rep(i,0,4*MAX)cout<<T[i]<<' ';cout<<endl;
            rep(i,1,n+1){
            int l = max((i-k),base);
            int r = min(i+k,n+1);
            //cout << l << r << endl;
            v.pb(query(1,0,n+1,l,r));
        }
